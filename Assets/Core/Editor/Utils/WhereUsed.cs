...
public static class WhereUsed  {
	...
	public static bool IsSelectionAssets ()
	{
		...
		{
			if ( EditorUtility.IsPersistent( selectedObject ) == false )
				return false;
		}
		return true;
	}
	...
	static HashSet<string> GetUnreferencedAssets()
	{
		...
		Func<string,bool> isResourceOrScene = (string p) => (p.StartsWith( "Assets/Resources" ) || p.EndsWith( ".unity" ));
		...
		otherAssets.Remove( referencesOutsideResources );
		otherAssets.Remove( LibraryReferences() );
		EditorUtility.ClearProgressBar();
		...
		if ( obj is Texture2D )
		{
			foreach ( var target in EnumUtil.GetValues<BuildTargetGroup>() )
			{
				foreach ( var icon in  PlayerSettings.GetIconsForTargetGroup( target ) )
				{
					if ( obj == icon )
					{
						foundLibraryReferences.Add( libraryPath + "/" + target.ToString() +"/Icons"  );
					}
				}
			}
		}
...
static class HashsetExtensions
{
	...
	public static void Remove( this HashSet<string> self, HashSet<string> toRemove )
	{
		var rm = new RemoveNotIncludedPredicate( toRemove );
		self.RemoveWhere( rm.Validate );
	}

	class RemoveNotIncludedPredicate
	{
		HashSet<string> hash = null;

		public RemoveNotIncludedPredicate(HashSet<string> hash )
		{
			this.hash = hash;
		}
